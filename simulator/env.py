...
class WaymaxBaseEnv(PlanningAgentEnvironment):
    def __init__(
        ...
        env_config: EnvironmentConfig,   # 这里的envconfig就是代表控制的对象，以及最大的对象数等等。
...
class WaymaxBicycleEnv(WaymaxBaseEnv):
    def __init__(
        ...
        dynamics_model = InvertibleBicycleModel(normalize_actions=normalize_actions)   # normalize就是将action的范围控制在[-1,1]之间
    ...
    def step(self, env_state: EnvState, action: jax.Array) -> Transition:
        ...
        print("next_simulator_state:",next_simulator_state)
