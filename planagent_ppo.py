...
if __name__ == "__main__":
    ...
    agent = Agent(waymax_env).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # ALGO Logic: Storage setup
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device)   # 修改维度，single_observation_space.shape
    actions = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)    # 修改维度，single_observation_space.shape
    ...
    next_obs, _ = waymax_env.reset(next(scenarios))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            #-------waymax action excute process------
            action_spec = waymax_env.action_spec()
            action = datatypes.Action(
                data=jnp.array(action),
        ...
        # flatten the batch
        b_obs = obs.reshape((-1,) + envs.single_observation_space.shape)    # 修改维度，single_observation_space.shape
        b_logprobs = logprobs.reshape(-1)
        b_actions = actions.reshape((-1,) + envs.single_action_space.shape)  # 修改维度，single_action_space.shape
